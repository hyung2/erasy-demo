...
export function computeSurface(rows: ScoreRowV2[]): AxisScore {
  ...
  const total = rows.length;
  // 이 축을 잴 근거가 하나라도 있으면 측정으로 본다.
  //
  //   근거는 두 갈래다. 하나는 **사용 이력**(휴면·방치를 판단), 다른 하나는 **미인지 상태**
  //   (몰랐던 계정이 있다는 사실 자체). 뒤엣것은 날짜가 없어도 관측된다 — 위 루프에서
  //   discovered 인자가 독립 인자로 따로 곱해지는 이유와 같다.
  //
  //   전에는 `coveredCount > 0`만 봤다. 그래서 소셜 연결목록처럼 **플랫폼이 마지막 사용
  //   시각을 주지 않는 입력만** 가진 사용자는, 몰랐던 계정을 아무리 많이 찾아 줘도 축이
  //   통째로 미측정으로 빠졌다. 실측에서 위험 신호 3건을 가진 사용자와 신호가 하나도 없는
  //   사용자가 똑같이 0점으로 나왔다(2026-08-25).
  //
  //   "몰랐던 계정을 찾아 준다"가 이 제품의 핵심 주장인데, 그 관측이 점수에서 사라지고
  //   있었다. 모르는 것(사용 시각)을 위험으로 세지 않는 것과, 아는 것(미인지 계정)을
  //   세지 않는 것은 다른 문제다. 앞의 것만 고치고 뒤를 두면 반대편으로 기운다.
  //
  //   coverage 분자는 그대로 둔다 — 그 수는 "사용 이력을 확인한 계정"이라는 뜻이고,
  //   미인지 계정을 거기 섞으면 확인하지 않은 것을 확인했다고 말하게 된다.
  const measured = coveredCount > 0 || discoveredN > 0;
 ...
 * 평평한 천장을 만든다. 그러면 조치 전후가 같은 값이 되어 "정리를 끝내면 몇 점"과
 * 조치별 상승폭이 전부 0이 된다 — 조치가 실제로 줄인 위험을 산식이 표현하지 못한다
 * (2026-08-28 상한안 배포에서 실제로 발생 — 투영 줄 소멸·기대상승 전부 +0).
 *
 * 뺄셈은 before·after에 같은 값이 걸려 상쇄되므로 **상승폭이 raw 그대로 살아난다**.
 * 축을 하나 재게 되면 W가 올라 감점 자체도 줄어든다. 다만 그 몫이 곧 상승은 아니다 —
 * 재는 순간 그 축의 실제 점수가 함께 들어오기 때문이다. 유출 대조는 유출이 없으면
 * 100점 축이 들어와 크게 오르지만, 위생은 재사용·2FA 실태가 그대로 반영돼 대개
 * 내려간다(H=60이면 −4). 그래서 대조만 회복 레버로 세운다(SSOT 5.5).
 */
